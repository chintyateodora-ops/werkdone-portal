...
/** Values align with screening registration `sourceType` (mammobus / HPV / FIT). */
const SCREENING_SOURCE_TYPE_OPTIONS = [
  'Event',
  'Campaign',
  'Referral',
  'Social Media',
...
/** Matches screening registration `reg-subsidies` CHAS options. */
...
export function ProspectDetail({ onNavigate, prospectRef }: ProspectDetailProps) {
  ...
  const handleCreateTask = (taskData: { name: string; dueDate: string; dueTime: string }) => {
    const combined = combineDdMmYyyyAndTime(taskData.dueDate, taskData.dueTime);
    const when = combined ?? new Date();
    const newTask = {
      id: Date.now().toString(),
      name: taskData.name,
      dueDate: when.toLocaleDateString('en-US', {
        month: 'short',
        day: 'numeric',
        year: 'numeric',
      }),
      dueTime: when.toLocaleTimeString('en-US', {
        hour: 'numeric',
        minute: '2-digit',
        hour12: true,
      }),
      status: when < new Date() ? ('overdue' as const) : ('upcoming' as const),
      completed: false,
    };
  ...
  const handleAddContactLog = (logData: {
    ...
    const combined = combineDdMmYyyyAndTime(logData.date, logData.time);
    const when = combined ?? new Date();
    const newLog = {
      id: Date.now().toString(),
  ...
  useEffect(() => {
    ...
    const sections = [
      ...
      { id: 'consent', element: document.getElementById('consent') },
      ...
      <div className="flex-1 overflow-auto">
        <div className="bg-white">
          ...
          <div className="px-6 pb-6 flex items-center gap-3">
            <span style={{ fontSize: 'var(--text-sm)', fontWeight: 'var(--font-weight-normal)', color: '#6B7280' }}>
              {prospect.gender} • {prospect.age} years • Next Review: {detailsFormData.nextReviewDate || '—'}
            </span>
            {detailsFormData.riskLevel && (
              <span
                ...
                style={{
                  backgroundColor:
                    detailsFormData.riskLevel === 'High'
                      ? '#FEE2E2'
                      : detailsFormData.riskLevel === 'Medium'
                        ? '#FEF3C7'
                        : '#DBEAFE',
                  color:
                    detailsFormData.riskLevel === 'High'
        ...
        <div className="p-6">
          ...
          {activeTab === 'details' && (
            <div className="flex gap-6">
              ...
              <div className="flex-1 relative">
                ...
                <div className="space-y-6">
                  {/* Personal Information — same fields & order as screening registration */}
                  <section id="personal" className="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <EditableField
                        ...
                        onChange={(value) => setDetailsFormData({ ...detailsFormData, name: value })}
                        required
                      ...
                      <EditableSelect
                        ...
                        emptyOptionLabel="Select Residential Status"
                      />
                      ...
                      <EditableSelect
                        ...
                        options={['Chinese', 'Malay', 'Indian', 'Eurasian', 'Others']}
                        emptyOptionLabel="Select Race"
                      />

                      <EditableSelect
                        label="Religion"
                        value={detailsFormData.religion}
                        isEditing={isEditingDetails}
                        onChange={(value) => setDetailsFormData({ ...detailsFormData, religion: value })}
                        options={[...RELIGION_OPTIONS]}
                        emptyOptionLabel="Select Religion"
                      />
                  ...
                  {/* Healthier SG & Subsidies — maps to screening registration `reg-subsidies` */}
                  <section id="screening" className="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <EditableSelect
                        ...
                        value={detailsFormData.chasCardType}
                        isEditing={isEditingDetails}
                        onChange={(value) => setDetailsFormData({ ...detailsFormData, chasCardType: value })}
                        options={[...CHAS_CARD_TYPE_OPTIONS]}
                        emptyOptionLabel="Select CHAS Card Type"
                      />
                      ...
                      <EditableField
                        label="Year of Last Screening"
                        value={detailsFormData.lastScreeningYear}
                        isEditing={isEditingDetails}
                        onChange={(value) =>
                          setDetailsFormData({ ...detailsFormData, lastScreeningYear: value })
                        }
                  ...
                  <section id="risk" className="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div className="flex flex-col gap-6">
                      <EditableSelect
                        ...
                        emptyOptionLabel="Select Risk Level"
                      />

                      <EditableTextArea
                        label="Personal History of Cancer"
                        value={detailsFormData.personalCancerHistory}
                        isEditing={isEditingDetails}
                        onChange={(value) =>
                          setDetailsFormData({ ...detailsFormData, personalCancerHistory: value })
                        }
                        rows={4}
                      ...
                      <EditableTextArea
                        ...
                        value={detailsFormData.preExistingConditions}
                        isEditing={isEditingDetails}
                        onChange={(value) =>
                          setDetailsFormData({ ...detailsFormData, preExistingConditions: value })
                        }
                        rows={4}
                      ...
                      <EditableTextArea
                        ...
                        value={detailsFormData.familyHistory}
                        isEditing={isEditingDetails}
                        onChange={(value) => setDetailsFormData({ ...detailsFormData, familyHistory: value })}
                        rows={4}
                      ...
                      <div className="flex items-start gap-3">
                        <Checkbox
                          id="risk-eligibility-check"
                          checked={detailsFormData.cancerScreeningEligibilityCheck === 'Yes'}
                          onCheckedChange={(c) =>
                            setDetailsFormData({
                              ...detailsFormData,
                              cancerScreeningEligibilityCheck: c === true ? 'Yes' : 'No',
                            })
                          }
                  ...
                  {/* Engagement — maps to screening registration Engagement section */}
                  <section id="engagement" className="bg-white rounded-lg border border-gray-200 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <EditableSelect
                        ...
                        options={[...SCREENING_SOURCE_TYPE_OPTIONS]}
                        emptyOptionLabel="Select Source Type"
                      />
                      <EditableField
                        label="Campaign / Event Name"
                        value={detailsFormData.sourceName}
                        isEditing={isEditingDetails}
                        onChange={(value) => setDetailsFormData({ ...detailsFormData, sourceName: value })}
                        placeholder="e.g. Pink for Life 2025"
                      />
                    </div>
                  </section>

                  {/* Consent — maps to screening registration Consent section */}
                  <section id="consent" className="bg-white rounded-lg border border-gray-200 p-6">
                    <h2 className={FORM_CARD_SECTION_TITLE_CLASS}>
                      CONSENT
                    </h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <EditableField
                        label="PDPA Consent"
                        value={detailsFormData.pdpaConsent}
                        isEditing={isEditingDetails}
                        onChange={(value) => setDetailsFormData({ ...detailsFormData, pdpaConsent: value })}
                      />

                      <EditableField
                        label="eDM Subscription"
                        value={detailsFormData.edmSubscription}
                        isEditing={isEditingDetails}
                        onChange={(value) => setDetailsFormData({ ...detailsFormData, edmSubscription: value })}
                      />

                      <EditableTextArea
                        label="Consent for SCS to Contact"
                        value={detailsFormData.consentContact}
                        isEditing={isEditingDetails}
                        onChange={(value) => setDetailsFormData({ ...detailsFormData, consentContact: value })}
                        rows={3}
          ...
          {activeTab === 'screening' && (
            <div className="space-y-6">
              <section id="screening-review" className="bg-white rounded-lg border border-gray-200 p-6">
                <h2 className={FORM_CARD_SECTION_TITLE_CLASS}>REVIEW</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <EditableSelect
                    label="Review period"
                    value={detailsFormData.reviewPeriod}
                    isEditing
                    onChange={(value) =>
                      setDetailsFormData({ ...detailsFormData, reviewPeriod: value })
                    }
                    options={[
                  ...
                  <EditableField
                    ...
                    value={detailsFormData.nextReviewDate}
                    isEditing={false}
                    onChange={() => {}}
                  />
                </div>
              </section>
              <section id="screening-history" className="bg-white rounded-lg border border-gray-200 p-6">
                <h2 className={FORM_CARD_SECTION_TITLE_CLASS}>
                  SCREENING HISTORY
                </h2>
